...
This repository includes a CMake build for Windows (`CMakeLists.txt`), tested for MSVC and MinGW toolchains.

1. Install CFITSIO and make sure `fitsio.h` and `cfitsio.lib`/`libcfitsio.a` are discoverable.
   - You can set `CMAKE_PREFIX_PATH` to the CFITSIO install prefix.
2. Configure:
   - `cmake -S . -B build -DCMAKE_BUILD_TYPE=Release`
3. Build:
   - `cmake --build build --config Release`
4. Binaries are generated in:
   - MSVC multi-config: `build/Release/`
   - MinGW single-config: `build/`

Expected tools:
- `hotpants`
- `extractkern`
- `maskim`
- `extractkernOnes` (optional, controlled by `-DBUILD_EXTRACTKERNONES=ON/OFF`)
